export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try { uids = req.body.uids || []; } catch (e) { uids = []; }
...
async function checkSingle(uidRaw) {
  ...
  try {
    ...
    if (res.status === 404) return { uid, status: 'error' };
    ...
    if (lower.includes('confirm your identity') ||
    ...
    if (lower.includes("this content isn't available") ||
